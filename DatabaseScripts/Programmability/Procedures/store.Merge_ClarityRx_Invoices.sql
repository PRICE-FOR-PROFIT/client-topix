...
GO
CREATE PROC [store].[Merge_ClarityRx_Invoices]
...
SELECT FileSource = CAST('raw.ClarityRx_Invoices' as NVARCHAR(100))
...
DELETE FROM raw.ClarityRx_Invoices
WHERE  TRY_CAST(NULLIF(TRIM([request_dt]),'NULL') AS DATE) IS NULL and [request_dt] IS NOT NULL
*/
...
SELECT
	    [InvoiceID]				= 'ClarityRx_' + ISNULL(NULLIF(TRIM([ord_no]),'NULL'),'')		+ ' | ' 
	  ...
	  ,[CustomerID]				= 'ClarityRx_' + ISNULL(NULLIF(TRIM(cus_no),'NULL'),'')
	  ,[ItemID]					= 'ClarityRx_' + ISNULL(NULLIF(TRIM([item_no]),'NULL'),'')
...
*/
/*
DECLARE @FromDate AS VARCHAR(10) = (SELECT CAST(MIN(CAST(inv_dt AS DATE)) AS VARCHAR(10)) from raw.ClarityRx_Invoices)
DECLARE @ToDate AS VARCHAR(10) = (SELECT CAST(MAX(CAST(inv_dt AS DATE)) AS VARCHAR(10)) from raw.ClarityRx_Invoices)
...
TRUNCATE TABLE raw.ClarityRx_Invoices
*/
