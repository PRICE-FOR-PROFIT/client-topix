...
GO
CREATE PROC [store].[Merge_Think_Customers]

AS

TRUNCATE TABLE store.Think_Customers

INSERT INTO store.Think_Customers (
 [CustomerID]				
,[cus_no]					
,[cus_name]				
,[addr_1]					
...
SELECT  [CustomerID]				
	   ...
	   ,[Company]				
FROM (
	SELECT [CustomerID]						= 'Think_' + CAST(NULLIF(TRIM([cus_no]),'NULL')	AS VARCHAR(100))
		  ...
		  ,[Company]							='Think'
		  ,R= ROW_NUMBER() OVER (PARTITION BY cus_no ORDER BY start_dt desc)
	FROM raw.Think_Customers
) x
WHERE R = 1
GO