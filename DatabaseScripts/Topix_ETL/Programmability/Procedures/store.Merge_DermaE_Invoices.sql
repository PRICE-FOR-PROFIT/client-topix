...
GO
CREATE PROC [store].[Merge_DermaE_Invoices]
...
SELECT FileSource = CAST('raw.DermaE_Invoices' as NVARCHAR(100))
,	Reason = CAST('Shifting of columns' as NVARCHAR(300))
,	LoadDate = GETDATE()
,   *
FROM raw.DermaE_Invoices
WHERE  TRY_CAST(NULLIF(TRIM([request_dt]),'NULL') AS DATE) IS NULL and [request_dt] IS NOT NULL

DELETE FROM raw.DermaE_Invoices
WHERE  TRY_CAST(NULLIF(TRIM([request_dt]),'NULL') AS DATE) IS NULL and [request_dt] IS NOT NULL
*/

TRUNCATE TABLE store.DermaE_Invoices

INSERT INTO store.DermaE_Invoices (
	   [InvoiceID]
      ,[CustomerID]
      ,[ItemID]
	  ,[orig_ord_type]
...
SELECT
	    [InvoiceID]				= 'DermaE_' + ISNULL(NULLIF(TRIM([ord_no]),'NULL'),'')		+ ' | ' 
	  ...
	  ,[CustomerID]				= 'DermaE_' + ISNULL(NULLIF(TRIM(cus_no),'NULL'),'')
	  ,[ItemID]					= 'DermaE_' + ISNULL(NULLIF(TRIM([item_no]),'NULL'),'')
...
*/
/*
DECLARE @FromDate AS VARCHAR(10) = (SELECT CAST(MIN(CAST(inv_dt AS DATE)) AS VARCHAR(10)) from raw.DermaE_Invoices)
DECLARE @ToDate AS VARCHAR(10) = (SELECT CAST(MAX(CAST(inv_dt AS DATE)) AS VARCHAR(10)) from raw.DermaE_Invoices)
...
TRUNCATE TABLE raw.DermaE_Invoices
*/
