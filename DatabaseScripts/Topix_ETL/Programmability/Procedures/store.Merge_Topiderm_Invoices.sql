...
GO
CREATE PROC [store].[Merge_Topiderm_Invoices]
...
SELECT FileSource = CAST('raw.Topiderm_Invoices' as NVARCHAR(100))
,	Reason = CAST('Shifting of columns' as NVARCHAR(300))
,	LoadDate = GETDATE()
,   *
FROM raw.Topiderm_Invoices
WHERE  TRY_CAST(NULLIF(TRIM([request_dt]),'NULL') AS DATE) IS NULL and [request_dt] IS NOT NULL

DELETE FROM raw.Topiderm_Invoices
WHERE  TRY_CAST(NULLIF(TRIM([request_dt]),'NULL') AS DATE) IS NULL and [request_dt] IS NOT NULL
*/

TRUNCATE TABLE store.Topiderm_Invoices

INSERT INTO store.Topiderm_Invoices (
	   [InvoiceID]
      ,[CustomerID]
      ,[ItemID]
	  ,[orig_ord_type]
...
SELECT
	    [InvoiceID]				= 'Topiderm_' + ISNULL(NULLIF(TRIM([ord_no]),'NULL'),'')		+ ' | ' 
	  ...
	  ,[CustomerID]				= 'Topiderm_' + ISNULL(NULLIF(TRIM(cus_no),'NULL'),'')
	  ,[ItemID]					= 'Topiderm_' + ISNULL(NULLIF(TRIM([item_no]),'NULL'),'')
...
*/
/*
DECLARE @FromDate AS VARCHAR(10) = (SELECT CAST(MIN(CAST(inv_dt AS DATE)) AS VARCHAR(10)) from raw.Topiderm_Invoices)
DECLARE @ToDate AS VARCHAR(10) = (SELECT CAST(MAX(CAST(inv_dt AS DATE)) AS VARCHAR(10)) from raw.Topiderm_Invoices)
...
TRUNCATE TABLE raw.Topiderm_Invoices
*/
