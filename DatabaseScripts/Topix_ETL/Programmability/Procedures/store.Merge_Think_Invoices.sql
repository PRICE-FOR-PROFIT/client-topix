...
GO
CREATE PROC [store].[Merge_Think_Invoices]
...
SELECT FileSource = CAST('raw.Think_Invoices' as NVARCHAR(100))
,	Reason = CAST('Shifting of columns' as NVARCHAR(300))
,	LoadDate = GETDATE()
,   *
FROM raw.Think_Invoices
WHERE  TRY_CAST(NULLIF(TRIM([request_dt]),'NULL') AS DATE) IS NULL and [request_dt] IS NOT NULL

DELETE FROM raw.Think_Invoices
WHERE  TRY_CAST(NULLIF(TRIM([request_dt]),'NULL') AS DATE) IS NULL and [request_dt] IS NOT NULL
*/

TRUNCATE TABLE store.Think_Invoices

INSERT INTO store.Think_Invoices (
	   [InvoiceID]
      ,[CustomerID]
      ,[ItemID]
      ,[ord_type]
...
SELECT
	    [InvoiceID]				= 'Think_' + ISNULL(NULLIF(TRIM([ord_no]),'NULL'),'')		+ ' | ' 
	  ...
	  ,[CustomerID]				= 'Think_' + ISNULL(NULLIF(TRIM(cus_no),'NULL'),'')
	  ,[ItemID]					= 'Think_' + ISNULL(NULLIF(TRIM([item_no]),'NULL'),'')
...
*/
/*
DECLARE @FromDate AS VARCHAR(10) = (SELECT CAST(MIN(CAST(inv_dt AS DATE)) AS VARCHAR(10)) from raw.Think_Invoices)
DECLARE @ToDate AS VARCHAR(10) = (SELECT CAST(MAX(CAST(inv_dt AS DATE)) AS VARCHAR(10)) from raw.Think_Invoices)
...
TRUNCATE TABLE raw.Think_Invoices
*/
